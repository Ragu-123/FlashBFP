...
class OABFLinear(torch.nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        3. Run Sparse Outliers SpMV (sparse path) using optimized 1D fast-path.
        4. Add Bias.
        """
        ...
        # Fast Outlier SpMV Path using Cached GPU Indices (Optimized 1D In-place Fast Path)
        if self._sparse_meta['count'] > 0 and self._sp_r_idx is not None:
            ...
            if c_idx.numel() > 0:
                M = X_flat.shape[0]
                if M == 1:
                    # 1D lookup & in-place update (no allocations, fast 1D indexing kernel)
                    X_active = X_flat[0, r_idx]
                    products = X_active * v_val
                    Y_dense[0].index_add_(0, c_idx, products)
                else:
                    # Fallback for sequence prefill (M > 1)
